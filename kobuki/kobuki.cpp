...
#define MUTEX_ASSIGN_IMPL(type, label)                                         \
    ...
    {                                                                          \
        ...
        const int bytes_written =                                              \
          write(m_##label.efd, &buffer, sizeof(int64_t));                      \
        if (bytes_written != sizeof(buffer))                                   \
        {                                                                      \
            log_error("Could not write to " #type                              \
                      " label. Wrote %ld instead of %lu : %s",                 \
                      bytes_written, sizeof(buffer), strerror(errno));         \
            m_run = false;                                                     \
        }                                                                      \
    }
...
template <size_t N>
bool
create_efds(std::array<int, N>& efds)
{
    for (size_t i = 0; i < N; ++i)
    {
        ...
        if (efd == -1)
        {
            log_error("Could not create event file descriptor #%lu : %s", i,
                      strerror(errno));
            return false;
        }
        efds[i] = efd;
...
template <typename T>
T
prepare_message(protocol::Command type)
{
    T msg;
...
Kobuki*
Kobuki::create(const char* device)
{
    ...
    if (fcntl(fileno(file), F_SETFD,
              fcntl(fileno(file), F_GETFD, 0) | O_NOCTTY | O_NDELAY))
    {
...
Kobuki::Kobuki(FILE* file, const std::array<int, N_EFD>& efds)
  : m_file(file)
  , m_run(true)
  , m_cached_output(static_cast<protocol::DigitalOutput>(0))
{
...
bool
Kobuki::find_packet_header()
{
    constexpr uint8_t HEADER_VALS[] = { protocol::HEADER_0_VAL,
                                        protocol::HEADER_1_VAL };
    int matches = 0;
...
bool
Kobuki::process_packet(const char* buffer, uint8_t length)
{
    uint8_t offset;
    for (offset = 0; offset < length;)
    {
        ...
        auto subpayload =
          reinterpret_cast<const protocol::FeedbackSubPayloadHeader*>(buffer +
                                                                      offset);
        offset += sizeof(protocol::FeedbackSubPayloadHeader);
        if (offset >= length)
        {
            log_error(
              "Not enough remaining bytes for the subpayload : %d vs %d",
        ...
        switch (subpayload->type)
        {
            case protocol::Feedback::BasicSensorData:
                ret =
                  on_msg(*reinterpret_cast<const protocol::BasicSensorData*>(
                    subpayload));
                break;
            ...
            case protocol::Feedback::DockingIR:
                ret = on_msg(
                  *reinterpret_cast<const protocol::DockingIR*>(subpayload));
                break;
            case protocol::Feedback::InertialSensor:
                ret =
                  on_msg(*reinterpret_cast<const protocol::InertialSensorData*>(
                    subpayload));
                break;
            case protocol::Feedback::Cliff:
                ret =
                  on_msg(*reinterpret_cast<const protocol::CliffSensorData*>(
                    subpayload));
                break;
            case protocol::Feedback::Current:
                ret = on_msg(
                  *reinterpret_cast<const protocol::Current*>(subpayload));
                break;
            ...
            case protocol::Feedback::HardwareVersion:
                ret =
                  on_msg(*reinterpret_cast<const protocol::HardwareVersion*>(
                    subpayload));
                break;
            case protocol::Feedback::FirmwareVersion:
                ret =
                  on_msg(*reinterpret_cast<const protocol::FirmwareVersion*>(
                    subpayload));
                break;
            ...
            case protocol::Feedback::RawData3AxisGyro:
                ret =
                  on_msg(*reinterpret_cast<const protocol::RawData3AxisGyro*>(
                    subpayload));
                break;
            ...
            case protocol::Feedback::GeneralPurposeInput:
                ret = on_msg(
                  *reinterpret_cast<const protocol::GeneralPurposeInput*>(
                    subpayload));
                break;
            ...
            case protocol::Feedback::UDID:
                ret = on_msg(
                  *reinterpret_cast<const protocol::UniqueDeviceIdentifier*>(
                    subpayload));
                break;
            case protocol::Feedback::Reserved_10:
                break;
            case protocol::Feedback::ControllerInfo:
                ret = on_msg(*reinterpret_cast<const protocol::ControllerInfo*>(
                  subpayload));
                break;
            default:
                log_error("Unrecognized subpaylaod type %d",
                          static_cast<int>(subpayload->type));
                return false;
        }
...
uint8_t
Kobuki::checksum(uint8_t packet_length, const char* buffer)
{
...
bool
Kobuki::validate_checksum(uint8_t packet_length, const char* buffer)
{
    uint8_t cs;
...
Kobuki::spin()
{
    while (ok())
    {
        ...
        int bytes_read =
          fread(&packet_length, 1, sizeof(packet_length), m_file);
        ...
        {
            log_error("Could not read packet payload. Read %ld instead of %ld",
                      bytes_read, static_cast<int>(packet_length));
            m_run = false;
            return;
        }
...
std::string
Kobuki::get_hardware_version() const
{
    return m_hardware_version;
}

std::string
Kobuki::get_firmware_version() const
{
    return m_firmware_version;
}

UDID
Kobuki::get_udid() const
{
    return m_udid;
}
...
Kobuki::send_msg(const T& msg)
{
    ...
    const uint8_t cs =
      checksum(header.length, reinterpret_cast<const char*>(&msg));
...
void
Kobuki::set_motion(double velocity, double radius)
{
    auto msg =
      prepare_message<protocol::MotionMessage>(protocol::Command::Motion);
...
void
Kobuki::set_sound(double frequency, double duration)
{
    auto msg =
      prepare_message<protocol::SoundMessage>(protocol::Command::Sound);
...
void
Kobuki::set_sound_sequence(SoundSequence sequence)
{
    auto msg = prepare_message<protocol::SoundSequence>(
      protocol::Command::SoundSequence);
...
void
Kobuki::set_leds(bool led_1_green, bool led_1_red, bool led_2_green,
                 bool led_2_red)
{
    auto msg = prepare_message<protocol::GeneralPurposeOutput>(
      protocol::Command::GeneralPurposeOutput);
    msg.digital_output = m_cached_output;
    m_cached_output &= ~(protocol::DigitalOutput::LED_1_Red |
                         protocol::DigitalOutput::LED_1_Green |
                         protocol::DigitalOutput::LED_2_Red |
                         protocol::DigitalOutput::LED_2_Green);

    if (led_1_green)
        m_cached_output |= protocol::DigitalOutput::LED_1_Green;
    if (led_1_red)
        m_cached_output |= protocol::DigitalOutput::LED_1_Red;
    if (led_2_green)
        m_cached_output |= protocol::DigitalOutput::LED_2_Green;
    if (led_2_red)
        m_cached_output |= protocol::DigitalOutput::LED_2_Red;
...
bool
Kobuki::on_msg(const protocol::BasicSensorData& basic_sensor_data)
{
    BasicData basic_data;

    basic_data.timestamp_ms = basic_sensor_data.timestamp_ms;

    basic_data.left_data.bumped =
      basic_sensor_data.bumper & protocol::Side::Left;
    basic_data.right_data.bumped =
      basic_sensor_data.bumper & protocol::Side::Right;
    basic_data.center_data.bumped =
      basic_sensor_data.bumper & protocol::Side::Center;

    basic_data.left_data.wheel_dropped =
      basic_sensor_data.wheel_drop & protocol::Wheel::Left;
    basic_data.right_data.wheel_dropped =
      basic_sensor_data.wheel_drop & protocol::Wheel::Right;

    basic_data.left_data.cliff_sensed =
      basic_sensor_data.cliff & protocol::Side::Left;
    basic_data.right_data.cliff_sensed =
      basic_sensor_data.cliff & protocol::Side::Right;
    basic_data.center_data.cliff_sensed =
      basic_sensor_data.cliff & protocol::Side::Center;
    ...
    basic_data.buttons.set(0, basic_sensor_data.button &
                                protocol::Button::Button_0);
    basic_data.buttons.set(1, basic_sensor_data.button &
                                protocol::Button::Button_1);
    basic_data.buttons.set(2, basic_sensor_data.button &
                                protocol::Button::Button_2);

    basic_data.is_charged =
      basic_sensor_data.charger &
      static_cast<protocol::Charger>((protocol::Charger::DockingCharged |
                                      protocol::Charger::AdapterCharged));
    basic_data.is_charging =
      basic_sensor_data.charger &
      static_cast<protocol::Charger>((protocol::Charger::DockingCharging |
                                      protocol::Charger::AdapterCharged));
    basic_data.is_using_adapter =
      basic_sensor_data.charger &
      static_cast<protocol::Charger>((protocol::Charger::AdapterCharged |
                                      protocol::Charger::AdapterCharging));
    basic_data.is_docked =
      basic_sensor_data.charger &
      static_cast<protocol::Charger>((protocol::Charger::DockingCharged |
                                      protocol::Charger::DockingCharging));

    basic_data.battery_voltage =
      protocol::BATTERY_VOLTAGE_RES * basic_sensor_data.battery_voltage;

    basic_data.left_data.overcurrent =
      basic_sensor_data.overcurrent & protocol::Wheel::Left;
    basic_data.right_data.overcurrent =
      basic_sensor_data.overcurrent & protocol::Wheel::Right;
...
bool
Kobuki::on_msg(const protocol::DockingIR& docking_ir)
{
    DockingIR ir;
...
bool
Kobuki::on_msg(const protocol::InertialSensorData& inertial_sensor_data)
{
...
bool
Kobuki::on_msg(const protocol::CliffSensorData& cliff_sensor_data)
{
...
bool
Kobuki::on_msg(const protocol::Current& current)
{
    Current cur;
...
bool
Kobuki::on_msg(const protocol::HardwareVersion& hardware_version)
{
    char buffer[256] = { 0 };
...
bool
Kobuki::on_msg(const protocol::FirmwareVersion& firmware_version)
{
    char buffer[256] = { 0 };
    snprintf(buffer, sizeof(buffer), "%d.%d.%d",
...
bool
Kobuki::on_msg(const protocol::RawData3AxisGyro& raw_gyro_data)
{
    GyroData gyro_data;

    const size_t number_of_entries = raw_gyro_data.n / 3;
    // Skipping all entries except the last one
    auto& raw_gyro_data_entry =
      *reinterpret_cast<const protocol::RawData3AxisGyroEntry*>(
        reinterpret_cast<const char*>(&raw_gyro_data) +
        sizeof(protocol::RawData3AxisGyro) +
...
bool
Kobuki::on_msg(const protocol::GeneralPurposeInput& gpi)
{
    GeneralPurposeInput general_purpose_input;

    general_purpose_input.digital_inputs.set(
      0, gpi.digital_input & protocol::DigitalInput::Channel_0);
    general_purpose_input.digital_inputs.set(
      1, gpi.digital_input & protocol::DigitalInput::Channel_1);
    general_purpose_input.digital_inputs.set(
      2, gpi.digital_input & protocol::DigitalInput::Channel_2);
    general_purpose_input.digital_inputs.set(
      3, gpi.digital_input & protocol::DigitalInput::Channel_3);
...
bool
Kobuki::on_msg(const protocol::UniqueDeviceIdentifier& udid)
{
...
bool
Kobuki::on_msg(const protocol::ControllerInfo& controller_info)
{
    PID pid;
...
void
Kobuki::request_identifiers()
{
    auto request_extra =
      prepare_message<protocol::RequestExtra>(protocol::Command::RequestExtra);
    request_extra.flags = protocol::RequestExtraFlag::HardwareVersion |
